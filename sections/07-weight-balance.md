# Weight and Balance

## Aircraft Identification

| Field | Value |
|-------|-------|
| Make | Vans |
| Model | RV-10 |
| Registration | N720AK |
| Serial | 41649 |

## Datum & Reference Arms

The datum is **99.44 inches forward of the wing leading edge**. All arms below are aft of datum.

| Item | Arm (in aft of datum) |
|------|----------------------|
| Wing Leading Edge | 99.44 |
| Pilot & Front Seat Passenger | 114.58 |
| Fuel | 108.90 |
| Rear Seat Passengers | 151.26 |
| Baggage | 173.50 |
| Main wheel, right | 123.12 |
| Main wheel, left | 123.13 |
| Nose wheel | 49.13 |

## Allowable CG Range

| Limit | Value |
|-------|-------|
| Forward Limit | 107.84" aft of datum |
| Aft Limit | 116.24" aft of datum |
| Range as % wing chord | 15% – 30% |
| Range from wing LE | 8.4" – 16.8" |

CG must remain within **107.84" – 116.24"** aft of datum at all loadings.

## Empty Weight & Balance

Aircraft was weighed empty in level flight attitude. **Includes 12 quarts of oil; no fuel.** Weighing date: **2025-11-18**.

| Station | Weight (lbs) | Arm (in) | Moment (lb-in) |
|---------|-------------:|---------:|---------------:|
| Right Wheel | 636 | 123.12 | 78,304 |
| Left Wheel | 648 | 123.13 | 79,788 |
| Nose Wheel | 359 | 49.13 | 17,638 |
| **Empty Total** | **1,643** | **106.96** | **175,730** |

**Empty CG = 175,730 / 1,643 = 106.96 in aft of datum.**

### Amendment 2026-09-08 (computed, no re-weigh)

Equipment installed after the 2025-11-18 weighing, revised by computation per
AC 43.13-1B Chapter 10. The amendment sheet is carried in the aircraft with the
weighing record.

| Change | Weight (lb) | Arm (in) | Moment (lb-in) |
|--------|------------:|---------:|---------------:|
| Empty weight as weighed 2025-11-18 | 1,643.0 | 106.96 | 175,730 |
| Monkworkz MZ-30 generator, regulator and BOSCH relay (2026-03-09), 34 in forward of the wing LE | +2.600 | 65.44 | +170.1 |
| OnSpeed AOA computer, 17 in forward of the wing LE (6 oz) | +0.375 | 82.44 | +30.9 |
| Borla 203133 regulator replacing the Aeromotive unit (2026-03-17), like-for-like | 0 | — | 0 |
| **Amended empty weight, 2026-09-08** | **1,646.0** | **106.89** | **175,931** |

**Amended empty CG = 175,931 / 1,646.0 = 106.89 in aft of datum.** Useful
load at 2,700 lb gross is 1,054 lb. Re-weigh at the next condition inspection
or major modification.

## Weights and Capacities

| Limit | Value |
|-------|-------|
| Maximum Takeoff Weight (Gross) | 2,700 lbs |
| Maximum Landing Weight | 2,700 lbs |
| Maximum Baggage Weight | 150 lbs |
| Empty Weight (weighed 2025-11-18, amended 2026-09-08) | 1,646 lbs |
| Useful Load (at 2,700 gross) | 1,054 lbs |
| Fuel Capacity (Total) | 60 U.S. gal (30 per tank) |
| Usable Fuel | 59 U.S. gal (29.5 per tank) |
| Fuel Weight (full, avgas at 6.0 lb/gal) | 360 lbs |

## Sample Loadings

All four scenarios use the amended empty weight (1,646 lb @ 106.89") and assume 12 qts oil.

### Most Forward CG

| Item | Weight | Arm | Moment |
|------|-------:|----:|-------:|
| Aircraft | 1,646 | 106.89 | 175,931 |
| Fuel (5 gal) | 30 | 108.90 | 3,267 |
| Pilot | 170 | 114.58 | 19,479 |
| Baggage | 7 | 173.50 | 1,214 |
| **Total** | **1,853** | | **199,891** |
| **CG** | | **107.87 in** | (at the forward limit) |

> A light pilot with minimum fuel needs about 7 lb in the baggage area to stay
> at or aft of the 107.84" forward limit; with 4 lb the CG falls to 107.77".

### Most Aft CG

| Item | Weight | Arm | Moment |
|------|-------:|----:|-------:|
| Aircraft | 1,646 | 106.89 | 175,931 |
| Fuel (5 gal) | 30 | 108.90 | 3,267 |
| Pilot | 170 | 114.58 | 19,479 |
| Rear Seat Passenger | 54 | 151.26 | 8,168 |
| Rear Seat Passenger | 150 | 151.26 | 22,689 |
| Baggage | 150 | 173.50 | 26,025 |
| **Total** | **2,200** | | **255,559** |
| **CG** | | **116.16 in** | (near the aft limit) |

### Max Gross — Typical 4-Up Loading

| Item | Weight | Arm | Moment |
|------|-------:|----:|-------:|
| Aircraft | 1,646 | 106.89 | 175,931 |
| Fuel (60 gal) | 360 | 108.90 | 39,204 |
| Pilot | 170 | 114.58 | 19,479 |
| Front Seat Passenger | 170 | 114.58 | 19,479 |
| Rear Seat Passenger | 170 | 151.26 | 25,714 |
| Rear Seat Passenger | 170 | 151.26 | 25,714 |
| Baggage | 14 | 173.50 | 2,429 |
| **Total** | **2,700** | | **307,950** |
| **CG** | | **114.06 in** | within limits |

### First Flight / Single Pilot Light Loading

| Item | Weight | Arm | Moment |
|------|-------:|----:|-------:|
| Aircraft | 1,646 | 106.89 | 175,931 |
| Fuel (30 gal) | 180 | 108.90 | 19,602 |
| Pilot | 175 | 114.58 | 20,052 |
| Baggage | 50 | 173.50 | 8,675 |
| **Total** | **2,051** | | **224,260** |
| **CG** | | **109.34 in** | within limits |

## Sample Calculation Worksheet

Use this template for actual flights. CG = Total Moment ÷ Total Weight.

| Item | Weight (lbs) | Arm (in) | Moment (lb-in) |
|------|-------------:|---------:|---------------:|
| Empty Aircraft | 1,646 | 106.89 | 175,931 |
| Pilot |  | 114.58 |  |
| Front Seat Passenger |  | 114.58 |  |
| Rear Seat Passenger |  | 151.26 |  |
| Rear Seat Passenger |  | 151.26 |  |
| Baggage (≤ 150 lb) |  | 173.50 |  |
| Fuel (× 6.0 lb/gal) |  | 108.90 |  |
| **Total** |  |  |  |

**CG must be between 107.84 and 116.24 inches aft of datum.**

**Total weight must not exceed 2,700 lbs.**

## Source Documents

- Authoritative W&B sheet: `Public/Weight-Balance/N720AK_WB_11_18_2025.pdf` on Google Drive (2025-11-18 weighing), with `N720AK_WB_amendment_2026-09-08.pdf` (computed amendment; generated by `scripts/wb_amendment.py`).
- A ForeFlight-compatible aircraft profile export is planned so that guest pilots can compute W&B for N720AK in ForeFlight.
