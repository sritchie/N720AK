# Brakes & Wheels

> ATA Chapter 61 — N720AK Systems Reference

## Overview

N720AK has a fixed tricycle landing gear with hydraulic disc brakes on the main wheels, operated by toe pedals on both pilot and co-pilot rudder pedals. The aircraft uses **Royco 782 (MIL-PRF-83282)** hydraulic fluid — not standard automotive brake fluid.

## Components

| Component | Part Number | Supplier | Notes |
|-----------|-------------|----------|-------|
| Brake calipers | <!-- TODO --> | <!-- TODO --> | Hydraulic disc |
| Master cylinders | <!-- TODO --> | <!-- TODO --> | Pilot and co-pilot toe pedals |
| Brake fluid | Royco 782 | — | MIL-PRF-83282 |
| Main wheels | WHLWI600XLT-2 | [Matco](https://drive.google.com/file/d/1ZMiXmYFD3oy5Z84GISiLJkfhUn_69s80/view) | WI600 series wheels & brakes |
| Nose wheel | AV-VANS-102-01 | [Beringer](https://drive.google.com/file/d/1-RQTx1cs9a9FDzubiTs9d8Xm1exrmfE1/view) | Beringer nosewheel (from finishing kit). Key parts: AV-VANS-004, JB-01, PAA02, AV-VANS-010. See [assembly drawing](https://drive.google.com/file/d/1mFLlQsnlvbyTMAaCnE1bWkoyrKa7Rsb3/view). |
| Main tires | 15x6.00-6, 6-ply | Desser | Tube-type retreads (inner tubes — see Tire Maintenance). Original pair: Goodyear PN 156E61-3 retreads by Aero Wheel & Brake Service Corp (Desser's Montebello repair station), 8130-3s #275723/275724, June 2016. Right main replaced 2026-08-30 with a 2024-date 15x6.00-6. Note: "15x6.00-6" is the short (~15" OD) variant — plain "6.00-6" is ~2.5" taller and will not fit the wheel pants; Desser "Monster" retreads in this size are also too large for the pants. |
| Nose tire | Michelin 5.00-5, 10-ply (5.00-5/10/120) | Michelin | PN 071-311-0, TSO-C62 (EASA Form 1 on file). Tubeless on the Beringer wheel (O-ring valve, Beringer A-001 replacement stem). |
| Main tubes | Aero Classic 15-600-6 TR-20 straight stem (GL-1520N) | Aircraft Spruce 06-01219 | Standard (non-Leakguard) butyl tube; one spare in stock. Angled-stem alternative: Desser TIRTU15x600x6/70 (GL-1587), Leakguard, TR-87 70° stem — the tube Matco specifies for the RV-10 with the Van's axle nut / wheel-pant bracket. |
| Wheel fairings | <!-- TODO --> | <!-- TODO --> | Main gear only |
| Axle extenders | RVAE10 (VAE10/RVbits) | Cleaveland Tool | Wheel pant axle extenders (pair). AN5-6A bolts, torque 100 in-lbs. See [installation guide](https://drive.google.com/file/d/1l-FiOyAeNCY5SBKun7PtulPUNS9CfxAR/view). |
| Wheel fairing spacers | WFSPCR | Planearound | Bracket spacers for wheel fairings |

## How It Works

### Brake System

Hydraulic disc brakes operated by toe pedals. Both pilot and co-pilot have independent toe brakes on their rudder pedals.

**Critical: Use only Royco 782 (MIL-PRF-83282) hydraulic fluid.** Do not use automotive brake fluid — it will damage the seals.

<!-- TODO: Brake line routing -->
<!-- TODO: Parking brake — is one installed? How does it work? -->

### Landing Gear

Fixed tricycle configuration:
- Steerable nose wheel
- Main gear legs (steel)
- Wheel fairings on main gear

**Nosewheel torque specs** (per [Beringer assembly drawing](https://drive.google.com/file/d/1mFLlQsnlvbyTMAaCnE1bWkoyrKa7Rsb3/view)): 6mm screw = 10 N.m (88 in.lb), 1/4" = 9 N.m (80 in.lb), 8mm = 20 N.m (177 in.lb), bearing contact ~25 N.m. Install locking wire and cotter pin. Check disc safety wire before every flight.

<!-- TODO: Nose wheel steering mechanism — direct link to rudder pedals? -->

## Inspection & Maintenance

### Brake Bleeding

<!-- TODO: Brake bleeding procedure — what worked, what didn't, lessons learned -->
<!-- TODO: Tools needed -->
<!-- TODO: How much fluid needed for a full bleed -->

### Tire Maintenance

| Specification | Value |
|---------------|-------|
| Main tire size | 15x6.00-6, 6-ply (the short ~15" OD variant — not plain 6.00-6) |
| Nose tire size | Michelin 5.00-5, 10-ply |
| Main tubes | Aero Classic 15-600-6 TR-20 straight (Spruce 06-01219); Desser GL-1587 TR-87 70° for pant valve access |
| Main tire pressure | 40 PSI |
| Nose tire pressure | <!-- TODO --> PSI |

**Tire source**: Desser standard-profile retreads (Aircraft Spruce carries them as "Retread Tire 15/600-6 6 Ply High Performance," P/N 06-01471) — [reference notes from another RV-10](http://www.myrv10.com/N104CD/upgrades/20110326/index.html). The mains are tube-type (Matco wheels, no tubeless kit); only the Beringer nosewheel is tubeless. Skip tires from other suppliers. **Avoid Monster retreads** — the extra tread depth grows the OD past wheel-pant clearance, and retreads expand further in service.
<!-- TODO: Tire replacement procedure -->

## References

- [Matco WI600 Series Wheel & Brake Manual (Rev A1)](https://drive.google.com/file/d/1ZMiXmYFD3oy5Z84GISiLJkfhUn_69s80/view)
- [Beringer Wheels & Brakes Maintenance Manual](https://drive.google.com/file/d/1-RQTx1cs9a9FDzubiTs9d8Xm1exrmfE1/view)
- [Beringer RV-10 Nosewheel Assembly Drawing (AV-VANS-102-01)](https://drive.google.com/file/d/1mFLlQsnlvbyTMAaCnE1bWkoyrKa7Rsb3/view) — includes maintenance manual + torque specs
- [RV-10 Axle Extension Replacement Installation (VAE10/RVbits)](https://drive.google.com/file/d/1l-FiOyAeNCY5SBKun7PtulPUNS9CfxAR/view) — AN5-6A bolts, 100 in-lbs torque, drill existing axle nuts to 5/16"
