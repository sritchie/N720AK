...
- **Recovering pump 1 after a cutover**: the auto-cutover latches on pump 2 — the bus manager does not switch back on its own. Cycling the pump selector switch **rapidly** (1/AUTO → 2 → 1/AUTO) resets the latch and re-activates pump 1. This is the recovery path if the system has cut over to pump 2 and pump 2 then fails (owner-verified 2026-08). <!-- TODO: confirm behavior on a ground run and note any timing sensitivity in the rapid toggle -->.
